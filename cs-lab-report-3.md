...
The instances of Lucayans have been highlighted by me above for ease of viewing, but on git bash, they are colored red so that the user may locate them. As evidenced here, the command has browsed through every file in written_2 and outputted the two results of "Lucayans" that it has found, both in the same file, `Bahamas-History.txt`. 
